...
class OIDCLoginFlask(OIDCLogin):
    ...
    def check_params(self) -> OIDCLogin:
        ...
        try:
            ...
            self.get_platform()
            self.validate_target_link_uri()
            return self
        except (err.MissingParameterError, err.ErrorException) as e:
            raise e
    ...
    def get_platform(self):
        ...
        """
        try:
            lms_url = os.environ.get('LMS_URL', 'https://moodle.haski.app')
            self._platform = self._tool_config.decode_platform(self._tool_config.get_platform(lms_url))
    ...
    def validate_target_link_uri(self):
        """Verify if the target_link_uri is valid and uses https in production
        and does not redirect to other domain than our tool"""
        target_link_uri = self._oidc_login_params_dict.get('target_link_uri')

        try:
            parsed_target_link_url = urllib.parse.urlparse(target_link_uri)
        except ValueError as e:
            raise err.ErrorException(e, message="target_link_uri is not a valid URL", status_code=400)
        ...
        try:
            ...
            if target_link_uri != self._platform.target_link_uri:
    ...
    def auth_redirect(self) -> Response:
        ...
        params = self.build_redirect_params(nonce, state_jwt)

        print(ru + urllib.parse.urlencode(params))
        response = redirect(ru + urllib.parse.urlencode(params))
        return response

    def build_redirect_params(self, nonce, state_jwt):
        return {
            'client_id': self._platform['client_id'],
    ...
    def verify_id_token(self) -> 'OIDCLoginFlask':
        ...
        if not id_token_jwt:
            self._response = jsonify(
                {'error': 'Invalid id_token, crypto key signature or LTI config data of LMS may have changed'}
            ), 400
            return self
        ...
        if not hmac_key:  # TODO try to get new keys, if that fails return error
            self._response = jsonify({'error': 'Invalid decryption key'}), 400
            return self

        self.id_token = JWTKeyManagement.verify_jwt(id_token_jwt, JWTKeyManagement.construct_key(hmac_key))
        if not self.id_token:
            self._response = jsonify({'error': 'Invalid id_token signature'}), 403
            return self

        try:
            self.parse_id_token()
        except Exception as e:
            self._response = jsonify({'error': 'Invalid id_token'}), 403
            return self
    ...
    def lti_launch_from_id_token(self) -> Response:
        ...
        nonce_jwt = JWTKeyManagement.generate_nonce_jwt(
            self.id_token.nonce, self._request.referrer,
        ...
        # get issuer
        # TODO🧾 based on issuer platform get corresponding implementation and write id token data into structures
        ...
        try:
            ...
            if not user:
                user = services.create_user(
                    ...
                    university=self.id_token['https://purl.imsglobal.org/spec/lti/claim/tool_platform']['name'],
                    lms_user_id=self.id_token.sub,
                    role=RoleMapper(self.id_token['https://purl.imsglobal.org/spec/lti/claim/roles']).get_role()
                )
    ...
    def get_login(self) -> Response:
        ...
        # use session service id_token to get user data and write into cookie
        ...
        response = self.build_login_response(token, user, nonce_payload)
        return response

    def build_login_response(self, token, user, nonce_payload):
