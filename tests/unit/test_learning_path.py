...
import domain.domainModel.model as domainModel
import pytest
...
def test_get_learning_path_correct_result():
    ...
    learning_path = ['ZF', 'UB', 'SE', 'AN',
                     'RQ', 'AB', 'ZL', 'BE', 'FO']
    graf_learning_path,List_LPLE = learning_path_calculated.get_learning_path(
                        input_learning_style)
                                
    assert graf_learning_path == learning_path
